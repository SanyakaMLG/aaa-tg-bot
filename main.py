...
async def game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    state[next(user_pick)][next(user_pick)] = CROSS

    if won(state):
        keyboard = generate_keyboard(state)
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.callback_query.edit_message_text(
            f'X (your) won!', reply_markup=reply_markup
        )
        return FINISH_GAME

    try:
        set_bot_choose(state)
    except IndexError:
        keyboard = generate_keyboard(state)
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.callback_query.edit_message_text(
    ...
    if won(state):
        await update.callback_query.edit_message_text(
            f'O (bot) won!', reply_markup=reply_markup
        )
        return FINISH_GAME
